...
import static pa.com.banistmo.userinterface.EditarComercioUserInterface.*;

public class EditarComercio implements Task {

    private final DatosComercio datosComercio;

public EditarComercio(DatosComercio datosComercio){
    this.datosComercio=datosComercio;
}

    public static EditarComercio enIpgMcs(DatosComercio datosComercio) {
        return Tasks.instrumented(EditarComercio.class,datosComercio);
    }

    @Override
    public <T extends Actor> void performAs(T actor) {

        actor.attemptsTo(Click.on(BOTON_EDITAR));
        if (TEXTO_MAESTRO.resolveFor(actor).isVisible()) {
            actor.attemptsTo(Enter.theValue(datosComercio.getMcc()).into(INPUT_TEXT_MCC));
            actor.attemptsTo(Enter.theValue(datosComercio.getNombreLegal()).into(INPUT_TEXT_NOMBRE_LEGAL));
            ...
            actor.attemptsTo(SelectFromOptions.byVisibleText(datosComercio.getMoneda()).from(OPCION_MONEDA));
            actor.attemptsTo(SelectFromOptions.byVisibleText(datosComercio.getZonaHoraria()).from(OPCION_ZONA_HORARIA));
            actor.attemptsTo(SelectFromOptions.byVisibleText(datosComercio.getEstado()).from(OPCION_ESTADO));
          ...
          if (TEXTO_DIRECCION.resolveFor(actor).isVisible()){
            ...
            actor.attemptsTo(Enter.theValue(datosComercio.getCodigoPais()).into(INPUT_TEXT_ESTADO));
            actor.attemptsTo(Enter.theValue(datosComercio.getCodigoPostal()).into(INPUT_TEXT_CODIGO_POSTAL));
            actor.attemptsTo(SelectFromOptions.byVisibleText(datosComercio.getCodigoPais()).from(OPCION_PAIS));
            actor.attemptsTo(Click.on(BOTON_SIGUIENTE));
        } else { return; }

          if (TEXTO_CONTACTO.resolveFor(actor).isVisible()) {
            actor.attemptsTo(SelectFromOptions.byVisibleText(datosComercio.getTipoContacto()).from(OPCION_TIPO_CONTACTO));
            actor.attemptsTo(Enter.theValue(datosComercio.getNombre()).into(INPUT_TEXT_NOMBRE));
            actor.attemptsTo(Enter.theValue(datosComercio.getCorreoContacto()).into(INPUT_TEXT_CORREO_CONTACTO));
        ...
        if (TEXTO_SERVICIOS.resolveFor(actor).isVisible()) {
            if(!TEXTO_RECURRING_PAYMENTS.resolveFor(actor).isVisible()){
                actor.attemptsTo(SelectFromOptions.byVisibleText(datosComercio.getServicio1()).from(OPCION_SERVICIO1));
                actor.attemptsTo(Click.on(BOTON_AGREGAR));
                actor.attemptsTo(Click.on(BOTON_SIGUIENTE));
            }
            if(TEXTO_CONNECT_HPP.resolveFor(actor).isVisible()&&TEXTO_CREDIT_CARD.resolveFor(actor).isVisible()&&TEXTO_PAYMENT_URL.resolveFor(actor).isVisible()&&TEXTO_RECURRING_PAYMENTS.resolveFor(actor).isVisible()){
                actor.attemptsTo(Click.on(BOTON_SIGUIENTE));
            }

        }else { return; }
        if (TEXTO_CONFIGURACION_TERMINAL.resolveFor(actor).isVisible()) {
            actor.attemptsTo(SelectFromOptions.byVisibleText(datosComercio.getPuntoFinal()).from(OPCION_PUNTO_FINAL));
        ...
        if (TEXTO_LIMITES_DE_COMPRA.resolveFor(actor).isVisible()) {
            actor.attemptsTo(Enter.theValue(datosComercio.getMontoLimite()).into(INPUT_TEXT_MONTO_LIMITE));
            actor.attemptsTo(Click.on(BOTON_AGREGAR_ACTUALIZAR));
            actor.attemptsTo(Click.on(BOTON_SIGUIENTE));
        }else { return; }
